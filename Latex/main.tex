...
% TODO: VERIFY AUTHOR NAME - Reviewer flagged "Tian Lvy" as potential typo
% Common alternatives: "Tian Lyu", "Tian Lv" - please confirm correct spelling
\author{Tian Lvy}
\affiliation{
  \institution{KAUST}
  \city{Thuwal}
...
\midrule
96  & 1.17{\scriptsize$\pm$0.03} & 1.12{\scriptsize$\pm$0.02} & 2.38{\scriptsize$\pm$0.05} & 26.0{\scriptsize$\pm$0.2} \\
104 & 1.54{\scriptsize$\pm$0.04} & 1.54{\scriptsize$\pm$0.04} & 2.75{\scriptsize$\pm$0.06} & 26.5{\scriptsize$\pm$0.2} \\
\textbf{107} & \textbf{2.14}{\scriptsize$\pm$0.06} & \textbf{2.14}{\scriptsize$\pm$0.06} & \multicolumn{1}{c}{N/A$^*$} & \textbf{27.0}{\scriptsize$\pm$0.2} \\
112 & 1.53{\scriptsize$\pm$0.04} & 1.53{\scriptsize$\pm$0.04} & 2.60{\scriptsize$\pm$0.05} & 27.1{\scriptsize$\pm$0.2} \\
128 & 1.47{\scriptsize$\pm$0.03} & 1.47{\scriptsize$\pm$0.03} & 2.55{\scriptsize$\pm$0.05} & 28.1{\scriptsize$\pm$0.2} \\
\bottomrule
...
\begin{tabular}{@{}llcc@{}}
\toprule
Architecture Type & SDPA head\_dim & \textbf{Repair Helps?} & Validated \\
\midrule
\textbf{Direct compression} & \emph{Misaligned} & \cellcolor{green!55}\textbf{\Large YES} & Kernel (\S\ref{sec:repair_validation}) \\
(vanilla SVD, head\_dim$\to d$) & (e.g., $d$=107) & \cellcolor{green!55}(+25--28\%) & \\
\midrule
\textbf{Projection-based} & \emph{Aligned} & \cellcolor{red!50}\textbf{\Large NO} & E2E (\S\ref{sec:e2e_validation}) \\
(RAP SVD, latent$\to$head\_dim) & (e.g., $d$=128) & \cellcolor{red!50}(--0.8\%) & \\
\midrule
\textbf{Quantization} & \emph{Unchanged} & \cellcolor{gray!40}\textbf{N/A} & N/A \\
(GPTQ, AWQ) & & & \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{End-to-end performance: Baseline vs.\ PaLU on Llama-3-8B ($B$=4, $S$=2048). PaLU achieves \textbf{11.5$\times$ decode speedup} via KV cache compression, with minimal prefill overhead (--2\%). Since production PaLU enforces 32-multiple alignment, no dimension repair is needed.}
\label{fig:e2e_perf}
\end{figure}
...
\midrule
107 (base) & 0\% & 2.06{\scriptsize$\pm$0.06} & 1.00$\times$ \\
...
120 & 1.56{\scriptsize$\pm$0.04} & 1.56{\scriptsize$\pm$0.04} & 1.43{\scriptsize$\pm$0.04} & 0\% & +8.3\% \\
...
\begin{enumerate}
  \item \textbf{Scope}: The 96.9\% misalignment figure is from theoretical Fisher-information analysis; all 24 production PaLU checkpoints enforce 32-multiple alignment. Our findings apply to methods without alignment constraints (vanilla SVD) or future methods that relax constraints for better compression.

  \item \textbf{Downstream Tasks}: Zero-padding preserves outputs exactly; perplexity validated on RAP SVD. Comprehensive task evaluation (MMLU, etc.) is future work.

  \item \textbf{Hardware Generalization}: All experiments are on A100. H100+ generalization remains future work.
\end{enumerate}
