...
\textbf{(3)}~End-to-end evaluation on Llama-3-8B with ASVD \cite{asvd} (SVD factorization) and LLM-Pruner \cite{llmpruner} (structured pruning), showing GAC achieves 100\% alignment with up to 1.5$\times$ prefill speedup (\S\ref{sec:eval}).
...
PyTorch's SDPA~\cite{pytorch_sdpa} leverages FlashAttention\nobreakdash-2 (FA2)~\cite{flashattention2}, MEM\_EFFICIENT, or MATH.
FA2 picks the smallest template ${\geq}\,d$ from \{64, 96, \ldots, 256\}; each boundary halves tile width $B_c$, doubling iterations.
MEM\_EFFICIENT needs $d \bmod 8 {=} 0$; otherwise MATH is used (up to $12{\times}$ slower).

\paragraph{Low-Rank and Pruning Methods.}
SVD methods~\cite{asvd,svdllm2024,fwsvd2022} decompose $W {\approx} U_r \Sigma_r V_r^T$; rank $r$ is set per layer and typically non-aligned.
...
All experiments run on NVIDIA A100-80GB with PyTorch 2.9.1, CUDA 12.8, FP16.
Latency uses CUDA event timing with 50 warmup and 200 measurement iterations across 3 trials.

\subsection{Dimension Distribution in Practice}
\label{sec:distribution}

We analyze unconstrained SVD rank allocation on Llama-3-8B ($r{=}0.8$) with four importance scoring methods across all projections (Figure~\ref{fig:dim_scatter}).
Across all combinations, 50--80\% of dimensions are not 8-aligned, confirming that misalignment is a \emph{structural property} of importance-based allocation, not an edge case.
...
We sweep each GEMM dimension independently while fixing others at typical LLM sizes ($M{=}2048$, $N{=}2048$, $K{=}128$).
...
\subsection{GeMV (Decode) Alignment}
\label{sec:gemv}

During autoregressive decode, each token generates a GeMV ($M{=}1$) rather than a GEMM.
Figure~\ref{fig:gemv_alignment} repeats the stride-1 sweep for GeMV.
Unlike GEMM, aligned and misaligned values track each other closely---no mod-8/16 comb pattern emerges.
The total variation (${\sim}$14\%) is far less than GEMM's 50--163\%.
GeMV is memory-bound; cuBLAS selects memory-optimized kernels that internally pad dimensions, masking Tensor Core effects.
GAC therefore targets \emph{prefill} (compute-bound GEMM), where alignment limits throughput.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_gemv_alignment.pdf}
\caption{\textbf{GeMV alignment sensitivity} ($M{=}1$, stride-1 sweep near 4096). Unlike GEMM (Figure~\ref{fig:gemm_alignment}), aligned and misaligned values show no periodic separation---the alignment penalty is negligible for memory-bound decode.}
