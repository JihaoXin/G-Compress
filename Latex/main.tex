...
\midrule
\textbf{Direct compression} & \emph{Misaligned} & \colorbox{green!30}{\textbf{\large Yes}} (+25--28\%) & Kernel (\S\ref{sec:repair_validation}) \\
(vanilla SVD, head\_dim$\to d$) & (e.g., $d$=107) & & \\
\midrule
\textbf{Projection-based} & \emph{Aligned} & \colorbox{red!25}{\textbf{\large No}} (--0.8\%) & E2E (\S\ref{sec:e2e_validation}) \\
(RAP SVD, latent$\to$head\_dim) & (e.g., $d$=128) & & \\
\midrule
\textbf{Quantization} & \emph{Unchanged} & \colorbox{gray!20}{\textbf{N/A}} & N/A \\
(GPTQ, AWQ) & & & \\
\bottomrule
\end{tabular}
\end{table}
