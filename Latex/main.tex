...
\caption{\textbf{[Theoretical Analysis]} Dimension distribution from \emph{unconstrained} Fisher-information-based rank allocation (Llama-3-8B, $r$=0.8). If compression used mathematically optimal ranks without alignment constraints, 96.9\% of 512 KV head dimensions would be misaligned. \textbf{Note:} All production PaLU checkpoints enforce 32-multiple alignment (\S\ref{sec:scope}); this figure motivates studying methods that relax such constraints.}
\label{fig:palu_dist}
\end{figure}
...
\caption{Speedup vs.\ memory overhead tradeoff for dimension repair. $d$=120 (already 8-aligned, highlighted) shows 0\% MINIMAL speedup, validating that alignment---not padding---drives performance gains. MINIMAL achieves 6.9$\times$ ROI (speedup/overhead); OPTIMAL provides 4.0$\times$ ROI.}
...
\textbf{(1) E2E integration gap}: Our kernel-level experiments (C4) demonstrate 25--30\% speedup for misaligned dimensions.
However, integrating repair into PaLU's factorized $W = U \cdot V^T$ structure requires adapting padding to the low-rank matrices---straightforward but not yet implemented.
Our RAP SVD validation produced $d$=102 (100\% misaligned), but the compression itself degraded perplexity significantly (11.08$\to$92.39), limiting its deployment value.
The kernel-level microbenchmarks remain our primary evidence; E2E integration with production-quality compressed models is future work.
