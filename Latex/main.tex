...
We propose \textbf{GAC} (GPU-Aligned Compression), a new compression \emph{paradigm} that imposes hardware alignment constraints on top of any dimension-reducing compressor.
GAC first identifies \emph{why} certain dimensions are slow through a full-stack analysis (\S\ref{sec:analysis}), then uses those insights to constrain a multi-choice knapsack optimizer that re-selects aligned dimensions under the same parameter budget.
GAC is a compressor-agnostic framework with a post-processing step that restores speed without changing the upstream compressor's accuracy-oriented design.
