...
\textbf{We validate with both positive and negative end-to-end cases}:
(1)~Positive: \textbf{86.9\% average speedup} across 45 real SDPA workloads (batch 1--8, sequences 512--2048, range 46--181\%) when SDPA operates on misaligned dimensions from direct compression.
(2)~Negative: \textbf{--0.8\%} for RAP SVD (projection-based architecture)---exactly as predicted by our framework, proving it correctly identifies when repair does \emph{not} help.
