...
Repair efficacy is \emph{architecture-dependent}: it helps when SDPA operates directly on compressed dimensions, but not for projection-based architectures---we provide an applicability framework (Table~\ref{tab:applicability}) that correctly predicts when repair applies, validated by our RAP SVD end-to-end experiments.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Applicability Framework}: We provide practitioner guidance (Table~\ref{tab:applicability}) predicting when dimension repair helps, validated by correctly predicting no benefit for projection-based architectures (RAP SVD: --0.8\%).
...
\midrule
96  & 1.17{\scriptsize$\pm$0.03} & 1.12{\scriptsize$\pm$0.02} & 2.38{\scriptsize$\pm$0.05} & 26.00{\scriptsize$\pm$0.20} \\
104 & 1.54{\scriptsize$\pm$0.04} & 1.54{\scriptsize$\pm$0.04} & 2.75{\scriptsize$\pm$0.06} & 26.50{\scriptsize$\pm$0.20} \\
\textbf{107} & \textbf{2.14}{\scriptsize$\pm$0.06} & \textbf{2.14}{\scriptsize$\pm$0.06} & \multicolumn{1}{c}{N/A$^*$} & \textbf{27.00}{\scriptsize$\pm$0.20} \\
112 & 1.53{\scriptsize$\pm$0.04} & 1.53{\scriptsize$\pm$0.04} & 2.60{\scriptsize$\pm$0.05} & 27.10{\scriptsize$\pm$0.20} \\
128 & 1.47{\scriptsize$\pm$0.03} & 1.47{\scriptsize$\pm$0.03} & 2.55{\scriptsize$\pm$0.05} & 28.10{\scriptsize$\pm$0.20} \\
\bottomrule
...
\begin{table}[t]
\centering
\caption{\textbf{KEY CONTRIBUTION: Applicability Framework}---When does dimension repair help? The critical factor is whether SDPA operates directly on compressed dimensions. \textbf{Practitioners: Use this decision table before applying repair.}}
\label{tab:applicability}
\begin{tabular}{@{}p{3.2cm}p{2.0cm}cc@{}}
\toprule
\textbf{Architecture Type} & \textbf{SDPA head\_dim} & \textbf{Repair Helps?} & \textbf{Validated} \\
\midrule
\rowcolor{green!15}
\textbf{Direct compression} & \emph{Misaligned} & \cellcolor{green!65}\textbf{\Large\checkmark~YES} & Kernel \\
\rowcolor{green!15}
(vanilla SVD, head\_dim$\to d$) & (e.g., $d$=107) & \cellcolor{green!65}\textbf{+25--28\%} & (\S\ref{sec:repair_validation}) \\
\midrule
\rowcolor{red!10}
\textbf{Projection-based} & \emph{Aligned} & \cellcolor{red!55}\textbf{\Large$\times$~NO} & E2E \\
\rowcolor{red!10}
(RAP SVD, latent$\to$head\_dim) & (e.g., $d$=128) & \cellcolor{red!55}\textbf{--0.8\%} & (\S\ref{sec:e2e_validation}) \\
\midrule
\rowcolor{gray!10}
\textbf{Quantization} & \emph{Unchanged} & \cellcolor{gray!35}\textbf{N/A} & --- \\
\rowcolor{gray!10}
(GPTQ, AWQ) & (original dims) & \cellcolor{gray!35}(no change) & \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Context: KV cache compression benefits (orthogonal to dimension repair).} Baseline vs.\ PaLU on Llama-3-8B ($B$=4, $S$=2048). The \textbf{11.5$\times$ decode speedup} comes entirely from \emph{reduced memory bandwidth} due to KV cache compression~\cite{palu}---\textbf{not from dimension repair}. Production PaLU enforces 32-multiple alignment internally, so dimensional collapse does not occur. This figure motivates why alignment-aware compression matters for future methods that may not enforce such constraints.}
\label{fig:e2e_perf}
\end{figure}
...
\subsection{Framework Validation: Correctly Predicting Non-Applicability}
\label{sec:e2e_validation}

A key strength of our applicability framework is its ability to predict \emph{when repair will NOT help}---preventing wasted engineering effort.
\textbf{Validation success}: We tested RAP SVD~\cite{rap} compressed Llama-3-8B ($d$=102, 100\% misaligned dimensions) and confirmed our framework correctly predicts no benefit for projection-based architectures.
Table~\ref{tab:rap_e2e} validates this prediction.
...
\textbf{H100 Considerations}: Our experiments focus on A100; H100/H200 validation is future work.
We note that FlashAttention-3 targets Hopper GPUs with optimized kernels for $d \in \{64, 128, 256\}$, and H100's 4th-generation Tensor Cores use similar tile sizes (m16n8k16 for FP16).
\emph{Whether similar dimensional collapse occurs on H100 requires empirical validation}---we do not claim generalization without data.
