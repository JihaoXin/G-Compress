...
We measured SDPA latency with inputs $Q,K,V$ of shape $(B, S, H, d)$: batch $B{=}4$, sequence length $S{=}2048$, number of heads $H{=}32$, and we sweep the per-head dimension $d$ from 64 to 256 (Table~\ref{tab:fa2_templates}).
...
\appendix
\onecolumn
\section{Compressed Dimension Across Compression Ratios}
\label{app:scatter_ratios}
The misalignment problem persists across different compression ratios.
Figure~\ref{fig:scatter_ratios} shows dimension scatter plots for Llama-3-8B under unconstrained SVD allocation at four compression levels ($\rho{=}10\%$, $20\%$, $30\%$, $50\%$) using Fisher importance scores.
