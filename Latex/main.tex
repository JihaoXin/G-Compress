...
Early work like AMC~\cite{amc2018} pioneered latency-constrained compression using reinforcement learning, achieving 1.95$\times$ mobile speedup by optimizing latency directly rather than FLOPs.
...
For LLM compression specifically, SVD-LLM~\cite{svdllm2024} achieves hardware speedups (3.1$\times$ on GPU at 80\% compression) through truncation-aware decomposition, while Fisher-weighted methods~\cite{fwsvd2022,gfwsvd2025} align compression with task importance rather than reconstruction error.
Low-Rank Prehab~\cite{lowrank_prehab2024} introduces pre-compression fine-tuning to encourage low-rank structure before applying SVD.
...
\textbf{Volta (2017)}: First-generation Tensor Cores required $K \bmod 8 = 0$ for FP16 MMA operations using m8n8k4 tiles~\cite{volta_whitepaper}.
\textbf{Ampere (2020)}: Second-generation cores tightened to $K \bmod 16 = 0$ for optimal m16n8k16 tiles~\cite{ampere_whitepaper}, doubling the alignment granularity and introducing 2:4 structured sparsity patterns~\cite{maskllm2024}.
\textbf{Hopper (2023)}: Third-generation cores introduced Tensor Memory Accelerator (TMA) with cache-line-aware access patterns (128B granularity)~\cite{nvidia_hopper_whitepaper,hopper_microbenchmark2024} and warpgroup execution (128 threads), potentially exacerbating alignment penalties.
...
FlashAttention-3~\cite{flashattention3} optimizes exclusively for dimensions $\{64, 128, 256\}$ on Hopper, \emph{removing} support for 96 and 112---likely due to Hopper-specific architectural constraints.
...
\textbf{Key insight}: Alignment requirements have become progressively stricter (8 → 16 → potentially 64), yet compression methods have largely ignored this trend.
Our work systematically documents how unconstrained compression violates these hardware contracts and provides diagnostic guidance for adapting to future architectures.
...
PaLU~\cite{palu} enforces 32-multiple alignment and implements optimized Triton kernels fusing key reconstruction, RoPE, and multiplication---but this design choice is absent from their paper, likely discovered through empirical profiling.
...
Unstructured pruning (SparseGPT~\cite{sparsegpt}) maintains dimensions but creates irregular sparsity patterns that limit GPU efficiency without specialized hardware.
...
we provide the first systematic analysis connecting compression-induced dimensional irregularities to GPU microarchitecture constraints (Tensor Core tiles, vectorized loads~\cite{memory_coalescing2024}, SDPA bandwidth).
...
\paragraph{Anticipating Criticisms and Positioning Our Work.}
\emph{If production systems already enforce alignment, why is this work needed?}
Our contribution is three-fold:
\textbf{(1)~Diagnostic guidance}: We provide the first systematic analysis of \emph{why} alignment matters through controlled hardware experiments (Tensor Core utilization 30\%$\to$12\%, vectorized load 50\% loss, SDPA bandwidth 40\% degradation, memory coalescing penalties~\cite{memory_coalescing2024}) rather than just documenting \emph{that} it matters.
Production systems discovered alignment constraints through trial-and-error profiling; we retroactively explain the root causes.
\textbf{(2)~Future-oriented framework}: We offer an applicability framework (Table~\ref{tab:applicability}) predicting when dimension repair helps versus when it doesn't---crucial for researchers evaluating new compression methods that may relax alignment constraints for accuracy gains.
\textbf{(3)~Validated predictions}: Our dual validation (RAP SVD --0.8\% vs Direct SDPA +86.9\%) demonstrates the framework's predictive power, preventing wasted engineering effort on architectures where repair provides no benefit.
...
Our experiments focus on A100 (Ampere architecture); H100 (Hopper) validation is future work.
Architectural similarities suggest dimensional collapse likely persists: H100's 4th-gen Tensor Cores use m16n8k16 MMA tiles requiring $K \bmod 16 = 0$~\cite{nvidia_hopper_whitepaper}.
However, H100 introduces new architectural features that may alter alignment sensitivity:
\textbf{(1)~TMA (Tensor Memory Accelerator)} performs cache-line-aware global-to-shared memory transfers with 128-byte granularity, potentially creating stricter alignment requirements or different performance cliffs.
\textbf{(2)~WGMMA instructions} operate on 64$\times$64 warpgroup tiles, suggesting $K \bmod 64$ may become optimal for maximum utilization.
\textbf{(3)~Different SM counts and memory hierarchy} (80 SMs vs. 108 SMs, 50MB vs. 40MB L2) may change the relative impact of our identified root causes (Tensor Core, vectorized loads, bandwidth).
Notably, FlashAttention-3~\cite{flashattention3} optimizes for $\{64, 128, 256\}$, \emph{removing} 96 and 112---possibly reflecting H100-specific constraints.
Our Shape Contract framework remains valid, but optimal alignment targets ($a=8$, $a=16$, or $a=64$) may require architecture-specific tuning.
Preliminary profiling on H100 would validate whether the diagnostic insights transfer or require adjustments.
