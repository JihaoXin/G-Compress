...
Existing dimension-reducing compressors allocate rank or width from importance scores and a global compression ratio, without hardware alignment constraints.
SVD and low-rank methods set per-layer rank $r_i$ from Fisher, magnitude, or activation scores; structured pruning sets output dimension $N$ from neuron importance; KV cache methods alter effective sequence length $M$.
Because these scores are continuous-valued, the chosen dimensions are typically \emph{irregular} (e.g., 107, 149, 305) and often violate GPU alignment requirements ($d \bmod 8 = 0$ or template boundaries), triggering backend fallbacks and kernel switches that erase the expected speedup from fewer FLOPs.

A concrete example is PaLU~\cite{palu} on Llama-3-8B at 20\% compression: unconstrained rank allocation yields head dimensions such as 107, 149, 305---none multiples of 8.
Figure~\ref{fig:palu_dist} shows the resulting per-layer rank distribution; a large fraction of layers end up misaligned (e.g., 78\% in this setup), so prefill latency can increase despite reduced parameter count.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig3_palu_dist.png}
\caption{PaLU on Llama-3-8B (20\% compression). Per-layer head dimension after unconstrained rank allocation; red = misaligned ($d \bmod 8 \neq 0$). A majority of layers are misaligned, leading to GPU performance cliffs.}
\label{fig:palu_dist}
\end{figure}

Budget allocation in current practice is driven by four families of importance scores: \textbf{magnitude} ($\|W_i\|_F$, SVD-LLM), \textbf{activation} ($\|X_i\|_F$, ASVD), \textbf{gradient} (Taylor-style, Taylor Pruning), and \textbf{Fisher} ($\mathrm{tr}(\mathbf{F}_i)$, PaLU, FWSVD).
All produce continuous-valued scores, so allocated ranks are naturally irregular.
Figure~\ref{fig:overview} summarizes the problem: unconstrained compression yields misaligned dimensions (e.g., $d{=}107$) and large latency increases; aligning to hardware-friendly values (e.g., $d{=}112$) recovers performance with minimal memory overhead.
Across four scoring methods and all projections of Llama-3-8B at $r{=}0.8$, 50--80\% of unconstrained dimensions are misaligned (Figure~\ref{fig:dim_scatter}).
...
\caption{Per-layer head dimension under unconstrained rank allocation (Llama-3-8B, $r{=}0.8$). Green = 8-aligned; red = misaligned. All four scoring methods (magnitude, activation, gradient, Fisher) produce 50--80\% misaligned dimensions.}
...
Root causes fall into three layers: \textbf{framework} (backend and template selection for SDPA), \textbf{library} (cuBLAS GEMM and GeMV kernel selection and alignment), and \textbf{hardware} (Tensor Core MMA tile fill, vectorized loads, L2 sector alignment).
Table~\ref{tab:constraints} summarizes the constraints and typical penalties; the following subsections detail each layer with microbenchmarks and figures.
...
The framework chooses the SDPA implementation and, for FlashAttention-2 (FA2), the tile template from head dimension $d$.
When $d \bmod 8 \neq 0$, PyTorch falls back to the MATH backend (up to $\sim$12$\times$ slower); when $d$ is 8-aligned but crosses FA2 template boundaries (e.g., 64, 96, 128, 160, 192, 224, 256), tile width $B_c$ halves and latency jumps.

We sweep \texttt{head\_dim} from 64 to 256 ($B{=}4$, $S{=}2048$, $H{=}32$).
Figure~\ref{fig:sdpa_latency} shows the staircase pattern: FA2 picks the smallest template $t \geq d$; $B_c$ halves at major boundaries (e.g., $128 \to 64 \to 32$), so crossing $d{=}128 \to 129$ causes $\sim$90\% latency increase.
Non-8-aligned dimensions trigger MATH fallback---e.g., $d{=}107$ incurs 2.14\,ms (+40\% vs $d{=}112$ at 1.53\,ms on Flash).
Table~\ref{tab:fa2_templates} quantifies the tiers.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig2_sdpa_latency.pdf}
\caption{SDPA latency vs.\ \texttt{head\_dim} (64--256). Green: Flash backend; red: Math fallback ($d \bmod 8 \neq 0$). Shaded regions show FA2 template tiers ($B_r \times B_c$). Cliff at $d{=}128 \to 129$ causes +90\% latency.}
...
\caption{Mistral-7B, $r$=0.8 (cross-model)}
\end{subfigure}
