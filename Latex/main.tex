...
\FloatBarrier  % Prevent earlier floats from drifting past this point
...
(1) Tensor Core utilization via \texttt{sm\_\_pipe\_tensor\_cycles\_active.avg.pct\_of\_peak\_sustained\_active};
(2) memory bandwidth via \texttt{dram\_\_throughput.avg.pct\_of\_peak\_sustained\_elapsed};
(3) vectorized load efficiency via \texttt{l1tex\_\_t\_sectors\_pipe\_lsu\_mem\_global\_op\_ld.avg.pct\_of\_peak\_sustained\_active}.
...
\FloatBarrier  % Prevent table drift before Section 6.1
...
\textbf{SVD methods}~\cite{svdllm2024,fwsvd2022,gfwsvd2025} optimize for reconstruction error or task accuracy without hardware-aware rank allocation---when unconstrained, Fisher-information-based ranks produce 96.9\% misaligned dimensions (\S\ref{sec:scope}).
