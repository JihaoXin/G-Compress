...
\begin{enumerate}
  ...
  \item \textbf{Evaluation}: Kernel-level experiments demonstrate 25--30\% speedup with 3.7\% memory overhead, yielding 6.9$\times$ ROI (\S\ref{sec:eval}).
\end{enumerate}
...
\subsection{PaLU Compression Benefits (Orthogonal Study)}

We compare Llama-3-8B baseline vs.\ PaLU-compressed (ratio=0.7) on A100 80GB to contextualize compression benefits separately from dimension repair.
...
\caption{\textbf{Compression benefit, not repair benefit.} Prefill and decode throughput for Llama-3-8B baseline vs.\ PaLU-compressed (ratio=0.7). The 11.5$\times$ decode speedup comes from reduced KV cache (compression effect), not dimension repair. These results are orthogonal to our alignment repair contribution.}
\label{fig:e2e}
\end{figure}
...
(3) \emph{E2E impact estimate}: Our kernel-level experiments validate 25--30\% speedup on SDPA operations. The actual E2E impact depends on attention's fraction of total inference time (typically 20--40\%), suggesting modest but worthwhile E2E improvements for compute-bound workloads. Precise E2E validation with synthetically misaligned models is left to future work.
...
End-to-end evaluation shows PaLU achieves 11.5$\times$ decode speedup from KV cache compression (orthogonal to our alignment contribution).
