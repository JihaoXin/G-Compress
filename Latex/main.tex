...
\paragraph{Sumamry} Table~\ref{tab:constraints} summarizes all constraints.
The minimum requirement across all layers is $d \bmod 8 = 0$; stricter alignment (mod~16, mod~32) yields further gains.
These penalties compound a misaligned dimension can trigger a backend fallback, a suboptimal kernel, and underutilized tiles simultaneously.
...
SVD-based methods~\cite{asvd,svdllm2024,palu,fwsvd2022,gfwsvd2025} allocate per-layer ranks via importance scores, producing irregular dimensions as a by-product.
...
Quantization~\cite{gptq,awq} preserves tensor dimensions via fixed-width groups, naturally avoiding dimensional misalignment.
GAC is complementary to all dimension-altering compressors: it post-processes their output to restore alignment.
...
Both treat latency as a black-box signal---they optimize aggregate runtime without isolating \emph{why} certain dimensions are slow.
In contrast, GAC identifies the root cause (dimensional misalignment across framework, library, and hardware layers) and constrains the search space accordingly, while guaranteeing the parameter budget.
...
These methods add memory and compute overhead but cannot change the model itself.
GAC prevents misalignment at compression time, eliminating the need for runtime padding or rejection.

\textbf{Limitations and future directions.}
Our evaluation covers two compressors, one model (Llama-3-8B), and one GPU (A100).
Alignment constraints grow stricter across GPU generations~\cite{nvidia_tensor_core_evolution2024}: Hopper introduces TMA with 128-byte transfers~\cite{nvidia_hopper_whitepaper}, and FlashAttention-3~\cite{flashattention3} drops support for \texttt{head\_dim} 96 and 112---suggesting the penalties we observe will only increase on newer hardware.
The current prototype operates as an offline post-processing pass; integrating GAC directly into compression libraries would make alignment-aware allocation automatic.
How alignment constraints interact when multiple techniques are composed (e.g., SVD + quantization) and whether a lightweight post-GAC finetuning pass can further recover quality at aggressive ratios remain open questions.
